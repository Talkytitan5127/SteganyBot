...
from telegram import KeyboardButton, ReplyKeyboardMarkup, ReplyKeyboardRemove, File
from telegram.ext import Updater, CommandHandler
from telegram.ext import MessageHandler, Filters, Dispatcher, CallbackQueryHandler, RegexHandler
import logging
...
class SteganyBot:
    ...
    def AnswerLog(self, bot, update):
        ...
        Answer += "\nFull Name => " + str(update.message.chat.first_name) + " " + str(update.message.chat.last_name)
    ...
    def AddHandler(self):
        ...
        self.dispatcher.add_handler(RegexHandler('^((E|e)ncrypt)|((D|d)ecrypt)',self.Choose))

        self.dispatcher.add_handler(CommandHandler("help", self.Help))

        self.dispatcher.add_handler(MessageHandler(Filters.text, self.GetText))

        self.dispatcher.add_handler(MessageHandler(Filters.document, self.GetDoc))
        
        self.dispatcher.add_handler(MessageHandler(Filters.photo, self.GetPhoto))

        self.dispatcher.add_handler(CommandHandler("Log", self.AnswerLog))
